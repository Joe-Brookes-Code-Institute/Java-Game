...
flashingInvaderSprites[0].src = 'assets/images/flashingSprite1.png';
flashingInvaderSprites[1].src = 'assets/images/flashingSprite2.png';
...
function update(deltaTime) {
    ...
    for (const invader of invaders) {
        if (invader.type === 2 && invader.status === 1) {
            invader.animationCounter += deltaTime;
            if (invader.animationCounter >= 500) { // Adjust this value to control animation speed
                invader.animationCounter = 0;
                invader.animationFrame = (invader.animationFrame + 1) % 2; // Toggle between 0 and 1
            }
        }
    }
    ...
    if (player.shootTimer >= player.shootInterval && player.ammo > 0 && player.isShooting) {
        ...
        bullets.push(createBullet(
            player.x + player.width / 2 - bulletSprite.width / 2,
            player.y,
            7 * speedMultiplier
        ));
        player.ammo--;
